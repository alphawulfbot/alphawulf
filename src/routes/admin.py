...
def sync_data():
    """Sync and validate all user data"""
    if not check_admin_auth():
        return jsonify({"error": "Unauthorized"}), 401
    
    try:
        logger.info("Admin initiated data sync")
        
        # Get all users
        response = supabase.table("users").select("*").execute()
        
        if not response.data:
            return jsonify({"message": "No users to sync"})
        ...
        for user_data in response.data:
            try:
                ...
                # Create User object to validate and fix data
                user = User(
                ...
                # Update energy and save
                user.update_energy()
                user.save()
                synced_count += 1
                
            except Exception as e:
                logger.error(f"Error syncing user {user_data.get('telegram_id', 'unknown')}: {str(e)}")
                error_count += 1
                continue
        
        logger.info(f"Data sync completed: {synced_count} users synced, {error_count} errors")
        
        return jsonify({
            "success": True,
            "message": f"Data sync completed",
